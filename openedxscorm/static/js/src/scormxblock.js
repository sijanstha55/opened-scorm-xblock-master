function ScormXBlock(runtime, element, settings) {
    ...
    var GetValue = function (cmi_element) {   
        ...
        if (uncachedValues.includes(cmi_element) && !navigationClick){
            ...
            navigationClick = false;
            returnValue = settings.scorm_data[cmi_element];
        }
        navigationClick = false;
        console.log('GetValue', cmi_element, returnValue);
        return returnValue;
    };
    ...
    var SetValue = function (cmi_element, value) {
        console.log('SetValue called:', cmi_element, value);
        // Always ensure completion status is properly tracked
        if (cmi_element === 'cmi.core.lesson_status' && (value === 'completed' || value === 'passed')) {
            console.log('Completion status detected:', value);
            // Force immediate processing for completion status
            var immediateData = [{
                'name': cmi_element,
                'value': value
            }];
            $.ajax({
                type: "POST",
                url: setValuesUrl,
                data: JSON.stringify(immediateData),
                async: false
            });
        }
    ...
    function processSetValueQueueItems() {
        ...
        while (setValueEvents.length > 0) {
            ...
            console.log('SetValue', cmi_element, value);
            
            // Force completion status to be sent to the server
            if (cmi_element === 'cmi.core.lesson_status' || 
                cmi_element === 'cmi.completion_status' || 
                cmi_element === 'cmi.success_status' || 
                cmi_element === 'cmi.progress_measure') {
                // Ensure this value is always sent to the server
                console.log('Important SCORM status value detected:', cmi_element, value);
            }
