...
class ScormXBlock(XBlock, CompletableXBlockMixin):
    ...
    def scorm_set_values(self, data_list, _suffix):
        logger.info("SCORM set_values: %s", data_list)
    ...
    def emit_completion(self, completion_percent):
        """
        Emit completion event to the LMS.
        This is a custom implementation since CompletableXBlockMixin may not be available.
        """
        try:
            # Try to use the completion API if available
            self.runtime.publish(self, 'completion', {'completion': completion_percent})
            logger.info(
                "SCORM emit_completion: published completion event with percent: %s",
                completion_percent
            )
        except (AttributeError, NotImplementedError) as e:
            # If completion API is not available, log the error
            logger.warning(
                "SCORM emit_completion: completion API not available: %s",
                str(e)
            )
    ...
    def set_value(self, data):
        ...
        logger.info("SCORM set_value: %s = %s", name, value)
        self.scorm_data[name] = value
        if name == "cmi.core.lesson_status":
            lesson_status = value
            logger.info("SCORM lesson_status set to: %s", lesson_status)
            if lesson_status in ["passed", "failed"]:
                success_status = lesson_status
                logger.info("SCORM success_status set to: %s", success_status)
            elif lesson_status in ["completed", "incomplete"]:
                completion_status = lesson_status
                logger.info("SCORM completion_status set to: %s", completion_status)
            # If lesson_status is 'passed', also mark as completed
            if lesson_status == "passed":
                completion_status = "completed"
                logger.info("SCORM completion_status set to 'completed' because lesson_status is 'passed'")
            ...
            logger.info("SCORM lesson_score set to: %s", lesson_score)
            ...
            logger.info("SCORM completion_percent set to: %s", completion_percent)
        ...
        if completion_percent is not None:
            ...
            logger.info("SCORM emitting completion with percent: %s", completion_percent)
        ...
        if completion_status == "completed":
            ...
            logger.info("SCORM emitting completion with percent: 1.0 due to completion_status='completed'")
        if success_status == "passed":
            self.emit_completion(1)
            logger.info("SCORM emitting completion with percent: 1.0 due to success_status='passed'")
        if (
            success_status
